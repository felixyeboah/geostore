  ...
  <div class="geo-modal__card" id="geo-qv-card" tabindex="-1"></div>
	...
	function bindSearch(input, opts) {
		...
		function render() {
			...
			/* Only suggest departments that have stock — suggesting an empty
			   one sends the visitor to a dead end. */
			const cats = q
				? (GEO.stockedCategories || GEO.categories).filter((c) =>
